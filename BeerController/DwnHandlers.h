...
if (current == &Program_Info) { current = &Welcome_Info; break; }    
    ...
    if (current == &SelectProgram_Info) { current = &Program_back_Info; break; }    
        if (current == &SelectProgram_Editor) { break; }
    if (current == &EditProgram_Info) { current = &SelectProgram_Info; break; }    
        if (current == &EditProgram_back_Info) { break; }
        if (current == &SelectStep_Info) { current = &EditProgram_back_Info; break; }    
            if (current == &SelectStep_Editor) { break; }
        if (current == &SelectOperation_Info) { current = &SelectStep_Info; break; }    
            if (current == &SelectOperation_Editor) { break; }
        if (current == &EditTemperature_Info) { current = &SelectOperation_Info; break; }    
            if (current == &EditTemperature_Editor) { break; }
        if (current == &EditMinutes_Info) { current = &EditTemperature_Info; break; }    
            if (current == &EditMinutes_Editor) { break; }
        if (current == &SavePrograms_Info) { current = &EditMinutes_Info; break; }    
            if (current == &SavePrograms_Editor) { break; }
    if (current == &RunProgram_Info) { current = &EditProgram_Info; break; }    
        if (current == &RunProgram_Editor) { break; }
    if (current == &ClearProgram_Info) { current = &RunProgram_Info; break; }    
        if (current == &ClearProgram_Editor) { break; }
    if (current == &ClearAllProgram_Info) { current = &ClearProgram_Info; break; }    
...
if (current == &Settings_Info) { current = &Manual_Info; break; }    
    ...
    if (current == &BounceDelay_Info) { current = &ReverseEnc_Info; break; }    
        if (current == &BounceDelay_Editor) { break; }
    if (current == &SaveSettings_Info) { current = &BounceDelay_Info; break; }    
